...
export async function saveListing(listing: MarketListing, s?: Store): Promise<void> {
  await (s ?? await openStore()).set(key(listing.regionId, listing.id), listing);
}
...
export async function listListings(regionId: string, s?: Store): Promise<MarketListing[]> {
  const entries = await (s ?? await openStore()).list<MarketListing>(["market", regionId]);
  return entries.map((e) => e.value);
}

/** Record a completed sale in the item's price history. */
export async function recordSale(listing: MarketListing, s?: Store): Promise<void> {
  const store = s ?? await openStore();
  const history = (await store.get<PricePoint[]>(historyKey(listing.itemId))) ?? [];
  history.push({ itemId: listing.itemId, price: listing.price, soldAt: new Date().toISOString() });
  if (history.length > PRICE_HISTORY_LIMIT) history.splice(0, history.length - PRICE_HISTORY_LIMIT);
  await store.set(historyKey(listing.itemId), history);
}

/** Recorded sales for an item, oldest first. */
export async function getPriceHistory(itemId: string, s?: Store): Promise<PricePoint[]> {
  return (await (s ?? await openStore()).get<PricePoint[]>(historyKey(itemId))) ?? [];
}
