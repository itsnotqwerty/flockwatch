...
Deno.test("successful espionage grants intel and slight suspicion", () => {
  ...
  assertEquals(out.player.intel.cleveland, 2);
...
Deno.test("a second flag in the same region restricts the player", () => {
  ...
  assert(twice.player.restricted.includes("cleveland"));
  assert(isRestricted(twice.player, "cleveland"));
...
Deno.test("espionage is refused above the suspicion cap", () => {
  const out = performEspionage(
    "tail",
    player({ suspicion: ESPIONAGE_SUSPICION_CAP }),
    region(),
    0.0,
  );
  assert(!out.ok);
